...
public class Int {
    ...
    private int[] producto(int[] n1, int[] n2){

        int[] result = new int[n2.length + n1.length];
        int[][] sumandos = new int[n2.length][n2.length + n1.length];
        for (int x = 1; x <= n2.length; x++){
             int acarreo = 0;
             int old_acarreo = 0;
             for (int y = 1; y <= n1.length; y++){
                old_acarreo = acarreo;
                acarreo = (n2[n2.length - x] * n1[n1.length - y] + old_acarreo) / 10 ;
                sumandos[x - 1][n2.length + n1.length - y - x + 1] = (n2[n2.length - x] * n1[n1.length - y] + old_acarreo - 10*acarreo);
                if (y == n1.length && acarreo != 0) {
                    sumandos[x - 1][n2.length + n1.length - y - x] = acarreo;
                }
            }
        }
